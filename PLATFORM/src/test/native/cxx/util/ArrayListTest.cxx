...
void testSort() {
	...
	DEEP_LOG(INFO, OTHER, "Display <Un>Sorted elements:\n");
	for (int i = 0; i < ARRAY_SIZE; i++) {
		DEEP_LOG(INFO, OTHER, "    %d = %lld\n", i,  al.get(i)->longValue());
	}

	Collections::sort(&al);

	DEEP_LOG(INFO, OTHER, "Display Sorted elements:\n");
	for (int i = 0; i < ARRAY_SIZE; i++) {
		DEEP_LOG(INFO, OTHER, "    %d = %lld\n", i,  al.get(i)->longValue());
	}
}
